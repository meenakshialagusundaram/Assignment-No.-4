...
data %>%
  ...
  geom_bar(stat = "identity", 
           na.rm = TRUE) +
  labs(title = "Analysing the the 2014 general elections in India", caption = "Source: Cambridge University Press", x = "Name of the State", y = "Percentage of BJP votes") +
  theme_bw() +
...
#Comparing poverty effect on female labor force participation in most rural vs most urban state in India through 2014-2017 
library(ggplot2)
library(dplyr)

state_year_summary <- data %>%
  filter(statename %in% c("Bihar", "Kerala")) %>%
  group_by(statename, ac_year) %>%
  summarize(mean_femalelfpr = mean(femalelfpr, na.rm = TRUE)) %>%
  ungroup()

state_year_summary$statecd <- as.factor(state_year_summary$statecd)
